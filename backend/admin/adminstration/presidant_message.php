...
<div class="main-content">
     <section class="section">
              <div class="form-header">
                  <h4>Presidant Message</h4>
              </div>
              <div class="form-section">
               <?php
                $data= $db->link->query("SELECT * FROM `presidant_message` WHERE `id`=1");
              ...
              if($data)
              {
                ...
                if(isset($_POST['save']))
                {
                    ...
                    $update=$db->link->query("UPDATE `presidant_message` SET `name`='$name',`title`='$title',`description`='$description' WHERE `id`=1");
                    if($update)
                    {
                     $file= $_FILES['image']['name'];
                     if($file)
                     {
                        $path='../../asset/img/presidant_message/'.$showdata['4'];
                        if(file_exists($path))
                        {
                            unlink($path);
                        }
                     ...
                     if($file)
                     {
                        ...
                        $image_path='../../asset/img/presidant_message/'.$image_name;

                        move_uploaded_file($_FILES['image']['tmp_name'],$image_path);

                        $db->link->query("UPDATE `presidant_message` SET `image`='$image_name' WHERE `id`=1");
               ...
               ?>
                <form method="POST" enctype="multipart/form-data">
                     ...
                     <div class="input-single-box">
                        ...
                        {
                        ?>
                        <img src="../../asset/img/presidant_message/<?php echo $showdata['4']; ?>" class="img-fluid" height="70px" width="90px">
                        <?php
                        }
                        ?>
                    </div>
